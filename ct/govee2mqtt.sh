...
echo -e "${INFO}Configuration Required:${CL}"
echo -e "${TAB}Copy example config: ${BGN}cp /opt/govee2mqtt/.env.example /opt/govee2mqtt/.env${CL}"
echo -e "${TAB}Edit configuration: ${BGN}nano /opt/govee2mqtt/.env${CL}"
echo -e "${TAB}Configure:"
echo -e "${TAB}• MQTT broker settings (GOVEE_MQTT_HOST, GOVEE_MQTT_PORT)"
echo -e "${TAB}• Govee credentials (GOVEE_EMAIL, GOVEE_PASSWORD, GOVEE_API_KEY)"
echo -e "${TAB}• Enable LAN control in Govee Home app for supported devices"
echo -e "${TAB}Start service: ${BGN}systemctl start govee2mqtt${CL}"
echo -e "${TAB}Check status: ${BGN}systemctl status govee2mqtt${CL}"
echo -e "${TAB}View logs: ${BGN}journalctl -fu govee2mqtt${CL}"
